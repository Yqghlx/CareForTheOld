...
class NeighborCircleService {
  ...
  Future<NeighborCircle?> getMyCircle() async {
    final response = await _dio.get('/neighborcircle/me');
  ...
  Future<NeighborCircle> createCircle({
    ...
    final response = await _dio.post('/neighborcircle', data: {
  ...
  Future<NeighborCircle> getCircle(String circleId) async {
    final response = await _dio.get('/neighborcircle/$circleId');
    final data = response.data['data'];
    return NeighborCircle.fromJson(data);
  }
  ...
  Future<List<NeighborCircleMember>> getMembers(String circleId) async {
    final response = await _dio.get('/neighborcircle/$circleId/members');
  ...
  Future<NeighborCircle> joinCircle(String inviteCode) async {
    final response = await _dio.post('/neighborcircle/join', data: {
  ...
  Future<void> leaveCircle(String circleId) async {
    await _dio.post('/neighborcircle/$circleId/leave');
  }

  /// 刷新邀请码（仅圈主可操作）
  Future<NeighborCircle> refreshInviteCode(String circleId) async {
    final response = await _dio.post('/neighborcircle/$circleId/refresh-code');
    final data = response.data['data'];
    return NeighborCircle.fromJson(data);
  }
