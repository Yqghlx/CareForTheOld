...
后端 292 个 xUnit 单元测试（186 Service + 68 Controller + 20 HealthAnomalyDetector + 18 其他），覆盖全部 Service 和全部 11 个 Controller。集成测试 12 个（需 Docker/Testcontainers）。测试项目：`CareForTheOld.Tests/`。
前端 390 个 Flutter 测试（表单验证、全模型序列化覆盖、AuthState 测试、OCR 解析器 27 个、VoiceParser 34 个、全部 18 个 Service 层 100% 覆盖、Provider 层 31 个、烟雾测试），使用 mocktail mock Dio。测试目录：`flutter_client/test/`。
