...
Models/Enums/         → 枚举定义
Common/Constants/     → 集中常量：AppConstants（业务数值）、ErrorMessages（错误消息字符串）
Common/Middleware/    → 三个中间件：异常处理、安全头、审计日志
Common/Helpers/       → 工具类（ApiResponse、GeoHelper、DbHelper 唯一约束判断、InviteCodeHelper 邀请码生成）
Common/Filters/       → Action 过滤器（CacheControlAttribute 缓存控制头）
Common/Extensions/    → 扩展方法（ServiceCollection、Controller、String 脱敏、HttpContext.GetClientIp 获取真实 IP）
...
- **客户端 IP 提取**：`HttpContextExtensions.GetClientIp()` 扩展方法，按优先级解析 `X-Forwarded-For` → `X-Real-IP` → `RemoteIpAddress`。限流和审计日志中获取真实 IP 时必须使用此方法，确保 Docker/Nginx 反向代理场景下 IP 正确。
...
- **集中常量管理**：`Common/Constants/AppConstants` 存放所有业务数值常量（围栏半径 500m、邀请码 7 天过期、用药提醒延迟、文件上传限制等），`Common/Constants/ErrorMessages` 存放所有用户可见错误消息（含 Auth、Family、Health、Medication、Emergency、User、GeoFence、NeighborCircle、NeighborHelp 等子类）。新增常量必须放入对应类中，禁止硬编码魔术数字或字符串。
- **文件上传常量**：`AppConstants.FileUpload` 集中管理头像上传限制（`AllowedAvatarExtensions`、`AllowedAvatarContentTypes`、`MaxAvatarSizeBytes`）。Controller 层通过引用这些常量校验文件，禁止硬编码文件类型或大小限制。
- **公共工具方法**：`Common/Helpers/DbHelper.IsUniqueConstraintViolation()` 统一判断唯一约束冲突（兼容 PG/SQLite），`Common/Helpers/InviteCodeHelper.Generate()` 生成 6 位数字邀请码。新增类似工具方法应放入 `Common/Helpers/`，不要在各 Service 中重复实现。
- **Hangfire 统一异步模式**：所有需要异步执行的代码（通知推送、SMS 发送、围栏告警等）统一使用 `BackgroundJob.Enqueue<TService>(svc => svc.PublicMethodAsync(...))` 或 `BackgroundJob.Schedule`。禁止使用 `Task.Run`、`_ = MethodAsync()` 等即发即弃模式，因为它们在进程重启后丢失且无法追踪。Hangfire 入口方法必须是 public，参数使用简单类型（Guid/string/int）以确保序列化。
- **审计日志**：Service 层关键操作（用户信息更新、密码修改、头像更新等）通过 `ILogger` 记录审计日志。日志内容包含操作者 ID 和操作描述，格式统一为 `_logger.LogInformation("用户 {UserId} 操作描述", userId)`。
...
│                   → constants/ 含 api_endpoints（集中 API 路径常量）
│                   → extensions/ 含 snackbar_extension、api_error_extension、date_format_extension（日期格式化）
...
- **API 路径常量化**：`core/constants/api_endpoints.dart` 集中管理所有 API 路径，静态字段用于固定路径，静态方法用于含参数的动态路径（如 `ApiEndpoints.familyMembers(familyId)`）。新增 API 调用必须使用常量，禁止在 Service/Provider 中硬编码路径字符串。
- **统一主题色**：`AppTheme` 中预定义了 `grey50`-`grey800` 灰度色和 `textTitle`/`textSubtitle`/`textCaption`/`textBody`/`textCardTitle` 文字样式。使用 `AppTheme.greyXXX` 替代 `Colors.grey.shadeXXX`，使用预定义 TextStyle 替代行内构建。
- **日期格式化扩展**：`date_format_extension.dart` 提供 `toDateString()`、`toTimeString()`、`toDateTimeString()`、`toShortDateTimeString()`、`toFriendlyDate()`、`toTimeRange()` 等方法。禁止在页面代码中手写 `DateFormat('yyyy-MM-dd').format(...)` 等重复格式化代码。
- **JsonSerializable 手写 fromJson**：当需要兼容后端返回的多种数据格式（如枚举可能是字符串或整数）时，使用 `@JsonSerializable(createFactory: false)` 抑制生成未使用的 `_$XxxFromJson`，手写 `fromJson` 工厂方法处理格式兼容。已应用于 `AnomalyEvent` 和 `TrendAnomalyDetectionResponse`。
- **资源释放**：持有 `Timer`、`StreamSubscription` 等资源的 Service 必须提供 `dispose()` 方法取消订阅和定时器。Provider 定义时需考虑资源生命周期。
