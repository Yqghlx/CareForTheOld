...
Services/Interfaces/  → 业务接口定义
Services/Implementations/ → 业务逻辑实现（12 个 Service）
...
Services/Hubs/        → SignalR NotificationHub（需 [Authorize]）
Services/Background/  → 后台服务（用药提醒定时任务）
```

### 关键后端模式

- **查询跟踪**：全局 `QueryTrackingBehavior.NoTracking`。写操作必须显式使用 `AsTracking()`，否则 `SaveChangesAsync` 不会更新实体。
- **认证**：JWT Bearer Token。密钥优先从环境变量 `JWT_SECRET_KEY` 读取，生产环境强制环境变量。
- **角色**：`UserRole.Elder`（老人）和 `UserRole.Child`（子女），控制器用 `[Authorize(Roles = "Child")]` 控制权限。
- **数据库**：开发环境 SQLite（`EnsureCreated`），生产环境 PostgreSQL（`Migrate`），测试 InMemory。
...
│   └── shared/     → 两端共用（通知、设置、紧急呼叫展示）
└── shared/         → 全局共享组件、模型、Provider（auth_provider）
```
...
152 个 xUnit 测试，覆盖全部 12 个 Service。测试项目：`CareForTheOld.Tests/`。
